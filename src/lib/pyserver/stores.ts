...
export const pyPackage = writable(import.meta.env.VITE_pypackage);

export const developerMode = writable(import.meta.env.DEV);

export const pyProgram = derived(
    [developerMode, pythonpath, pyPackage],
    ([$developerMode, $pythonpath, $pyPackage]) => {
        return $developerMode ? $pythonpath : $pyPackage;
    },
);
...
export const pyPackageName = writable('');
export const pyPackageArch = writable('');
...
export const serverCurrentStatus = writable<OutputBoxtype>({ value: 'python server not running', type: 'error' });
